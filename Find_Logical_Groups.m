...
function [Group_Start, Group_End] = Find_Logical_Groups(Input_Array)
    %% Input Handling
    if(nargin ~= 1)
        error("Find_Logical_Groups : Expected single logical array as input");
    end
    if(~islogical(Input_Array))
        error("Find_Logical_Groups : Expected logical array as input");
    end
    %% Validate array is the correct orientation
    Array_Size = size(Input_Array);
    if(numel(Array_Size) > 2)
        error("Find_Logical_Groups : Expected one dimensional array.");
    end
    ...
    if(Array_Size(1) == 1)
        if(Array_Size(2) == 1)
            error("Find_Logical_Groups : Expected one dimensional array wtih more than one element.");
        else
            Flip_Array = true;
        end
    else
    ...
    if(length(Group_Start) ~= length(Group_End))
        warning("Find_Logical_Groups : Unexpected error, differing lengths for Start and End.");
    end
end